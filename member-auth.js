...
function initSavedCards() {
  ...
  // "Come back to this": a Record entry joins My Practice on her word (MEMBER_LOBBY_PRD.md)
  function comeBackRow(card, item) {
    ...
    dayInput.addEventListener("change", () => { if (dayInput.value) send("date", dayInput.value); });
    const tomorrow = () => {
      const date = new Date();
    ...
    actions.append(
      ...
      option("Choose a day", () => { dayInput.hidden = false; dayInput.focus(); }),
...
const practiceAddDays = (day, count) => {
...
function practiceIcs(item) {
  ...
  const lines = [
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Practice Village//My Practice//EN",
    "BEGIN:VEVENT",
...
function initMyPractice() {
  ...
  function itemRow(item, today, compact = false) {
    ...
    if (!compact && item.href && item.linkLabel) {
      ...
      link.href = item.href;
      if (/^https?:/i.test(item.href)) { link.target = "_blank"; link.rel = "noopener"; }
      main.append(link);
    }
    if (!compact) {
      ...
      tools.append(calendar, document.createTextNode(" · "), remove);
      main.append(tools);
    }
    row.append(main);
    return row;
  ...
  function renderWeek(today) {
    ...
    for (let offset = 0; offset < 7; offset += 1) {
      ...
      cell.append(el("p", "practice-group", offset === 0 ? "Today" : new Date(y, m - 1, d).toLocaleDateString(undefined, { weekday: "short", day: "numeric" })));
  ...
  listButton.addEventListener("click", () => { if (view !== "list") { view = "list"; render(); act({ action: "set_view", view: "list" }); } });
  weekButton.addEventListener("click", () => { if (view !== "week") { view = "week"; render(); act({ action: "set_view", view: "week" }); } });
  ...
  return {
    refresh: async () => { try { const r = await practiceApi(); items = r.items; view = r.view; failed = false; } catch { failed = true; } render(); renderVillage(); },
    setVoucher: (covers) => { voucherCovers = Boolean(covers); renderVillage(); },
  };
}
...
async function initMemberLobby() {
  ...
  // quiet membership status, not a headline: Founding Villagers keep their name
  document.getElementById("memberPlan").textContent = roles.includes("founding_villager") ? "Founding Villager" : "Village Member";

  // the Concierge is simply ready: the working desk, embedded, no second welcome
      ...
      if (membership.onboardingStatus === "not_started") {
        ...
        if (offer) {
          ...
          document.getElementById("dismissInvite").addEventListener("click", async () => {
            ...
            try { await fetch("/member-onboarding", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "skip" }) }); } catch { /* the offer stays dismissed for this visit either way */ }
          });
        }
      }
...
function initDesk(user, opts = {}) {
  ...
  if (embedded) {
    // the member home: the working Concierge itself, no second welcome, no exits
  ...
  function renderEnd(keptCount) {
    const kept = keptCount
      ? `<p class="desk-q">${keptCount === 1 ? "One thing" : `${keptCount} things`} kept in your Record.${embedded ? "" : " It is in your lobby whenever you want it."}</p>`
  ...
  if (!embedded) {
    // Before you go: leaving the desk page with candidates pending offers the review once.
    // Never on the member home: she is already home, and home has no gates.
...
async function initWelcome() {
  ...
  // The Concierge lives on the member home now. /welcome is the orientation
  // surface only; a plain visit goes straight home. No extra click-throughs.
...
async function initRoomShell() {
  ...
  hushAdd?.addEventListener("click", async () => {
    ...
    try {
      await practiceApi({ action: "add_item", item: { title: "HUSH · 60 seconds", when: "daily", href: "https://hush-aidedeq.netlify.app/", linkLabel: "Open HUSH", source: "hush" } });
