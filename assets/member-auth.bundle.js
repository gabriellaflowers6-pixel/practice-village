...
function initDesk(user, opts = {}) {
  ...
  let last = null;
  let busy = false;
  const keepables = /* @__PURE__ */ new Map();
  const esc = (t) => {
  ...
  const keepButton = (key, entry) => {
    ...
    return isKept(entry) ? `<button type="button" class="text-button desk-keep" disabled>\u2713 kept for the wrap-up</button>` : `<button type="button" class="text-button desk-keep" data-keep-block="${esc(key)}">Keep this</button>`;
  };
  const closeLine = (d) => d.nextStep || d.searchHelp || d.results?.items?.length ? `<p class="desk-close">Would you like to keep any of this for your PIL?</p>` : "";
  function render(d) {
    keepables.clear();
    let html = bubbles();
    if (d) {
      ...
      if (d.results && d.results.items) {
        ...
        html += `<div class="desk-block"><span class="desk-label">${esc(d.results.title)}</span><ul class="desk-found">${d.results.items.map((it) => `<li><a href="${esc(it.href)}" target="_blank" rel="noopener">${esc(it.name)}</a> \xB7 ${esc(it.detail)}</li>`).join("")}</ul><p class="desk-note">${esc(d.results.sourceNote)}</p>${keepButton("results", entry)}</div>`;
      }
      if (d.searchHelp) {
    ...
    if (pending.length) html += `<p class="desk-pending">${pending.length} ${pending.length === 1 ? "thing" : "things"} set aside \xB7 <button type="button" class="text-button" data-wrap>wrap up and review</button></p>`;
